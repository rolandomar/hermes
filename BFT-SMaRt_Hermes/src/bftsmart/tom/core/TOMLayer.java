...
public final class TOMLayer extends Thread implements RequestReceiver {
    ...
    public TOMMessage[] checkProposedValue(byte[] proposedValue, boolean addToClientManager) {
        java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.isProposedValueValid) starting");
            ...
            if (addToClientManager) {
                for (int i = 0; i < requests.length; i++) {
                    ...
                    if (!clientsManager.requestReceived(requests[i], false)) {
                        ...
                        java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.isProposedValueValid) finished, return=false");
                        java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"failure in deserialize batch");
                        return null;
                    }
                }
            }
        ...
        java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.isProposedValueValid) finished, return=true");

        return requests;
    }
    public void forwardRequestToLeader(TOMMessage request) {
        java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,
                "(TOMLayer.forwardRequestToLeader) forwarding " + request);
        int leaderId = lm.getCurrentLeader();
        //******* EDUARDO BEGIN **************//
        if (this.reconfManager.isCurrentViewMember(leaderId)) {
            java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,
            "(TOMLayer.forwardRequestToLeader) forwarding " + request + " to " + leaderId);
            communication.send(new int[]{leaderId}, 
                new ForwardedMessage(this.reconfManager.getStaticConf().getProcessId(), request));
        }else{
            java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,
            "(TOMLayer.forwardRequestToLeader) forwarding failed leader not in currentiew:" + leaderId);
        }
            //******* EDUARDO END **************//
    }
    ...
    public void setNoExec() {
        java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.setNoExec) modifying inExec from " + this.inExecution + " to " + -1);
    ...
    public void triggerTimeout(List<TOMMessage> requestList) {
        java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.triggerTimeout)");
        ...
        if (lcManager.getNextReg() == lcManager.getLastReg()) {
                ...
                java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.triggerTimeout) initialize synch phase");
                ...
                try { // serialize content to send in STOP message
                    ...
                    // send STOP-message
                    java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.triggerTimeout) sending STOP message to install regency " + regency);
            ...
            java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.triggerTimeout) lcManager.getNextReg() != lcManager.getLastReg()");
    ...
    private void evaluateStops(int nextReg) {
        java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.evaluateStops): " + nextReg);
        ...
        if (lcManager.getStopsSize(nextReg) > this.reconfManager.getQuorumF() && lcManager.getNextReg() == lcManager.getLastReg()) {
           ...
           java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.evaluateStops) initialize synch phase");
           requestsTimer.Enabled(false);
            requestsTimer.stopTimer();
            ...
            try { // serialize conent to send in the STOP message
                ...
                // send message STOP
                java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,
                        "(TOMLayer.evaluateStops) sending STOP message to install regency " + regency);
                communication.send(this.reconfManager.getCurrentViewOtherAcceptors(),
                    new LCMessage(this.reconfManager.getStaticConf().getProcessId(), TOMUtil.STOP, regency, payload));
        ...
        if (lcManager.getStopsSize(nextReg) > this.reconfManager.getQuorum2F() && lcManager.getNextReg() > lcManager.getLastReg()) {
            ...
            java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,
                    "(TOMLayer.evaluateStops) installing regency " + lcManager.getNextReg());
            //Rolando
            ...
            requestsTimer.setTimeout(FailureDetector.recalculateTimeout(lastRegency,nextRegency,requestsTimer.getTimer()));
            java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.evaluateStops) newtimeout="+requestsTimer.getTimer());
            ...
            if (leader != this.reconfManager.getStaticConf().getProcessId()) {
                ...
                try { // serialize content of the SYNC message
                    ...
                    if (last > -1) { // content of the last decided eid
                        ...
                        if (exec.getDecisionRound() == null || exec.getDecisionRound().propValue == null) {
                            ...
                            java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"[DEBUG INFO FOR LAST EID #1]");
                            
                            if (exec.getDecisionRound() == null) 
                                java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"No decision round for eid " + last);
                            else System.out.println("round for eid: " + last + ": " + exec.getDecisionRound().toString());
                            if (exec.getDecisionRound().propValue == null) 
                                java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"No propose for eid " + last);
                            else {
                                java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"Propose hash for eid " + last + ": " + Base64.encodeBase64String(computeHash(exec.getDecisionRound().propValue)));
                            }
                    ...
                    java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.evaluateStops) sending STOPDATA of regency " + regency);
                ...
                java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.evaluateStops) I'm the leader for this new regency");
                ...
                if (last > -1) {  // content of the last decided eid 
                    ...
                    if (exec.getDecisionRound() == null || exec.getDecisionRound().propValue == null) {
                        ...
                        java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"[DEBUG INFO FOR LAST EID #2]");
            ...
            case TOMUtil.STOP: // message STOP

                {
                    ...
                    if (msg.getReg() == lcManager.getLastReg() + 1) {
                       ...
                       java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,
                               "(TOMLayer.deliverTimeoutRequest) received regency change request");
            ...
            case TOMUtil.STOPDATA: // STOPDATA messages
                {
                    ...
                    if (regency == lcManager.getLastReg() &&
                        ...
                        java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,
                                "(TOMLayer.deliverTimeoutRequest) I'm the new leader and I received a STOPDATA");
    ...
    private void catch_up(int regency) {
        ...
        java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,
                "(TOMLayer.catch_up) verify STOPDATA info");
        ObjectOutputStream out = null;
        ByteArrayOutputStream bos = null;
        ...
        if (lcManager.sound(lcManager.selectCollects(regency, currentEid))) {
            ...
            java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.catch_up) sound predicate is true");
            ...
            try { // serialization of the CATCH-UP message
                ...
                java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.catch_up) sending SYNC message for regency " + regency);
    ...
    private void finalise(int regency, LastEidData lastHighestEid,
        ...
        java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.finalise) final stage of LC protocol");
        ...
        if (getLastExec() + 1 < lastHighestEid.getEid()) {
            ...
            java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"NEEDING TO USE STATE TRANSFER!! (" + lastHighestEid.getEid() + ")");
        ...
        // Is this replica still executing the last decided consensus?
            ...
            java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"I'm still at the eid before the most recent one!!! (" + lastHighestEid.getEid() + ")");
        ...
        if (tmpval == null && lcManager.unbound(selectedColls)) {
            java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.finalise) did not found a value that might have already been decided");
            tmpval = propose;
        }
        else java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.finalise) found a value that might have been decided");

        if (tmpval != null) { // did I manage to get some value?
            
           java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.finalise) resuming normal phase");
            ...
            if (iAmLeader) {
                java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.finalise) wake up proposer thread");
                imAmTheLeader();
            } // waik up the thread that propose values in normal operation
            java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.finalise) sending WEAK message");
        ...
        else java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,"(TOMLayer.finalise) sync phase failed for regency" + regency);
    }
    /**************************************************************/
}
