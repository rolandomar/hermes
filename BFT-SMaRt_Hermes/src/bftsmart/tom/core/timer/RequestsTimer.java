...
public class RequestsTimer {
    ...
    public void startTimer() {        
        if (rtTask == null) {
            ...
            Logger.println("RequestsTimer:--------timeout----------> " + t);
             //Thread.dumpStack();
            //shortTimeout = -1;
    ...
    public void run_lc_protocol() {
        ...
        java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,
                "run_lc_protocol: "+t+ " short:"+shortTimeout+" timeout:"+timeout);
        ...
        if (!pendingRequests.isEmpty()) {
            for (ListIterator<TOMMessage> li = pendingRequests.listIterator(); li.hasNext(); ) {
                ...
                if (!request.timeout) {
                    ...
                    //do not remove, trigger timeout instead with 
            ...
            if (!pendingRequests.isEmpty()) {
                java.util.logging.Logger.getLogger(RequestsTimer.class.getName()).log(Level.SEVERE,
                        "run_lc_protocol: Timeout for messages: " + pendingRequests);
                //Logger.debug = true;
